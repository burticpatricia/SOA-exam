  ...
  return {
    option(name, factory) {
      if (providers.has(name)) {
        throw new Error(
          `Can not define an option with name "${String(name)}" since it is already used as the name of a provider.`,
        )
      }
    ...
    provide(name, factory) {
      if (options.has(name)) {
        throw new Error(
          `Can not define a provider with name "${String(name)}" since it is already used as the name of an option.`,
        )
      }
